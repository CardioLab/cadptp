...
{p2colset 5 20 30 2}{...}
{p2col:{hi:cadptp_cta} {hline 2}}CAD-PTP: Clinical likelihood of coronary artery disease defined using computerized tomography angiogram (CTA) {p_end}
{p2colreset}{...}
...
{pstd}
cadptp_cta using data in memory
...
{synopt:{opt suffix}(string)} Adds a suffix to the names of variables created by {cmd:cadptp_cta}{p_end}
{synopt:{opt replace}} Replaces variables created by {cmd:cadptp_cta} if they already exist{p_end}
...
{pstd}  {com}. use https://vbn.aau.dk/files/331937080/synthetic_cadptp_cta.dta
        {txt}(Synthetic dataset of patients with chronic coronary syndrome )
		{p_end}

{pstd}Run {cmd:cadptp_cta} using data in memory{p_end}
{phang2}{cmd:. cadptp_cta , male(sex) age(age) symp(symp_gr3) }

{pstd}Run {cmd:cadptp_cta} using data in memory including number of risk factors {p_end}
{phang2}{cmd:. cadptp_cta , male(sex) age(age) symp(symp_gr3) nb_rf(nb_rf_5 )  }

{pstd}Run {cmd:cadptp_cta} using data in memory including number of risk factors & CACS  {p_end}
{phang2}{cmd:. cadptp_cta , male(sex) age(age) symp(symp_gr3) nb_rf(nb_rf_5 )  cacs(calciumscoreagatston) }

{pstd}Run command, specifying the variable suffix {p_end}
{phang2}{cmd:. cadptp_cta , male(sex) age(age) symp(symp_gr3) nb_rf(nb_rf_5 )  cacs(calciumscoreagatston) suffix("_newStudy")}

{pstd}Run {cmd:cadptp_cta} using data in memory including number of risk factors & CACS  and  replace existing estimates{p_end}
{phang2}{cmd:. cadptp_cta , male(sex) age(age) symp(symp_gr3) nb_rf(nb_rf_5 )  cacs(calciumscoreagatston) replace }

{pstd}Run {cmd:cadptp_cta} using data in memory including number of risk factors & CACS, replace existing estimates and estimates the pre-test probability groups  {p_end}
{phang2}{cmd:. cadptp_cta , male(sex) age(age) symp(symp_gr3) nb_rf(nb_rf_5 )  cacs(calciumscoreagatston) replace  grp}
...
{pstd}
{cmd:cadptp_cta} stores the following in memory:
...
{marker citation}{title:Citation of {cmd:cadptp_cta}}

{p 4 8 2}{cmd:cadptp_cta} is not an official Stata command. It is a free contribution
to the research community, like a paper. Please cite it as such: {p_end}

{p 4 8 2}{it}
Schmidt SE & Winther S (2020). cadptp_cta: Stata module for calculating Clinical likelihood of coronary artery disease, github.com/CardioLab/cadptp_cta  {p_end} {reset}
