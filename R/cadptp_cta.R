#' The cadptp_cta calculates pre-test probability (PTP) of having obstructive coronary artery disease defined from CT scanning in patients with chronic coronary syndrome 
#' 
#' @description
#' The cadptp_cta command calculates pre-test probability (PTP) of having obstructive coronary artery disease defined from CT scanning in patients with chronic
...
#' 
#' Estimae PTP:
#' cadptp_cta(heart[,'sex'],heart[,'age'],heart[,'symp_gr3'],heart[,'nb_rf_5'],heart[,'calciumscoreagatston']) 


cadptp_cta <- function(sex, age , symp_gr3, nb_rf, cacs) {
  ...
  # Basic PTP
  ptp_basic = 1. / (1 + exp(-(
    -5.3575 + (0.7952 * sex) + (0.061684 * age) + (1.2329 * symp_typical) + (-0.58634 *
                                                                             symp_non_anglinal) + (-0.0054478 * age * symp_typical)
  )))
  ...
  ptp_rf = 1. / (1 + exp(-(
    -7.1076  + (1.1332 * sex) + (0.072819 * age) +  (1.4252 * symp_typical) + (-0.55152 *
                                                                               symp_non_anglinal) + (1.1145 * nb_rf_3) + (-0.005578 * age * symp_typical) + (-0.0079662 *
                                                                                                                                                             age * nb_rf_3) + (-0.11213 * symp_typical * nb_rf_3) + (-0.18948 * sex * nb_rf_3)
  )))
  ...
  ptp_cacs = 0.014533 + (ptp_rf * 0.27093) + (cacs_1_9 * 0.047056) + (cacs_10_99 * 0.1189) + (cacs_100_399 * 0.34417)  + (cacs_400_999 * 0.5881) + (cacs_1000 * 0.73892) + (ptp_rf * cacs_1_9 * 0.054423) + (ptp_rf * cacs_10_99 * 0.14347) + (ptp_rf * cacs_100_399 * 0.18439) + (ptp_rf * cacs_400_999 * 0.085774) + (ptp_rf * cacs_1000 * -0.081776)  
